...
static int __mt7925_init_hardware(struct mt792x_dev *dev)
{
	...
	ret = mt7925_mcu_set_deep_sleep(dev, dev->pm.ds_enable);
	if (ret) {
		dev_err(dev->mt76.dev, "Failed to configure deep sleep: %d\n", ret);
		goto out;
	}

	/* Initialize thermal protection early */
	ret = mt7925_mcu_set_thermal_protect(dev);
	if (ret)
		dev_warn(dev->mt76.dev, "Failed to setup thermal protection: %d\n", ret);

	/* Enhanced coexistence initialization */
...
int mt7925_register_device(struct mt792x_dev *dev)
{
	...
	INIT_DELAYED_WORK(&dev->phy.beacon_refresh_work, mt7925_beacon_refresh_work);
